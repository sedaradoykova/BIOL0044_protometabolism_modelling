...
```{r amino acid graphs no glut, fig.height=6, echo=FALSE, warning=FALSE, include = T}
par(mfrow = (c(3,2)))
...
#knitr::include_graphics("co2_fixation_pathway.png") ## pgn not available
```
...
``` {r gly graphs 2, warning=FALSE, echo=FALSE, include = T}
par(mfrow = (c(2,4)))
...
```{r co fixation no early nadph 2, include = T}
par(mfrow = (c(2,4)))
...
``` {r co fixation graphs co slow 2, include = T}
par(mfrow = (c(2,4)))
...
``` {r co fixation graphs more H+ 2, include = T}
par(mfrow = (c(2,4)))
...
``` {r co fixation graphs red agents 2, include=T}
par(mfrow = (c(2,4)))
...
``` {r co fixation graphs change rate constants 2, include=T}
par (mfrow = c(2,4))
...
``` {r co fixation graphs combined model 2, include=T}
par(mfrow = (c(2,4)))
...
```{r co fixation graphs vol, fig.height=6, include = T, warning=FALSE, echo=FALSE}
par(mfrow = (c(2,2)))
...
parms_k <- c(k_1 = 10, k_2 = 19.9, k_3 = 23.5, k_4 = 0.869, 
           k_5 = 0.061) 
...
# initial concentrations of reactants and products involved 
x0_no_cof <- c(CO2 = 3*nmol, H2 = 5*nmol, CO = 0, H2O = 0, formic = 0, 
               formaldehyde = 0, methanol = 0, acetic = 0, pyruvic = 0, 
               NH3 = 0.6*nmol, alanine = 0, oxaloacetic = 0, aspartic = 0, malic = 0, 
               fumaric = 0, succinic = 0, oxoglutaric = 0, glutamic = 0, acetolactic = 0, 
               dihydroxyisoval = 0, methyl_oxobut = 0, valine = 0, glycine = 0, x = 1)
